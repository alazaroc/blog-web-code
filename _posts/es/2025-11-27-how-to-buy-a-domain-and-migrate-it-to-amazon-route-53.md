---
layout: post
title: Cómo comprar un dominio y migrarlo a Amazon Route 53
...
lang-exclusive:
  - en
  - es
description: Aprende cómo comprar un dominio donde quieras y gestionarlo en AWS con Amazon Route 53 paso a paso.
category:
  - General
tags:
  - level-300
...
1. Comprarlo en AWS con **Amazon Route 53 Registered Domains**
...
- Tu dominio apuntando a Amazon Route 53
...
## 2. Crear la Hosted Zone en Amazon Route 53

Una vez que tengas el dominio, toca configurarlo en AWS:

1. Ve a la consola de **Amazon Route 53** → _Hosted zones_.
...
3. Amazon Route 53 te dará **4 servidores de nombres (NS)**. Estos son los que usarás más adelante en tu proveedor de dominios.

![2-amazon-route53-dns.png](2-aws-route53-dns.png)
...
ACM te generará un registro **CNAME** que tendrás que añadir en tu Hosted Zone de Amazon Route 53. Una vez validado, tu certificado quedará disponible.
...
3. Sustituye los valores que aparecen por los **4 NS de Amazon Route 53**.
...
Lo importante es que los NS que devuelve el comando coincidan con los que ves en la Hosted Zone de Amazon Route 53. Si todavía ves los NS antiguos de tu proveedor, es que la propagación no ha terminado.
...
- El registro DNS en Amazon Route 53 apuntando a esa distribución.
...
2. Añades el dominio como _Alternate domain name (CNAME)_.
...
### 5.2 Crear el registro DNS en Amazon Route 53

En la **Hosted Zone** de Amazon Route 53 crea un registro:
...
Una vez que tienes el dominio gestionado desde Amazon Route 53 y el certificado en ACM, conectar servicios de AWS a tu dominio se vuelve muy sencillo.
